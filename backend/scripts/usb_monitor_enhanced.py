...
print("=" * 70)
print("🛡️  SECURE FILE SHARING - USB DEVICE MONITOR (ENHANCED)")
print("=" * 70)
...
try:
    ...
    HAS_WATCHDOG = False
    print("⚠️  Warning: watchdog not installed. Install with: pip install watchdog")
...
try:
    ...
    HAS_WMI = False
    print("⚠️  Warning: WMI not available (Windows only library). USB detection may be limited.")
...
try:
    ...
    print("⚠️  Warning: face_recognition not installed. Face detection disabled.")
...
def get_active_user_id():
    ...
    try:
        ...
        print(f"⚠️  Could not read active user config: {e}")
    ...
    try:
        ...
        print(f"⚠️  Could not query database for admin user: {e}")
...
def load_known_faces():
    ...
    for face_file in face_files:
        try:
            ...
            print(f"❌ Failed to load face {face_file.name}: {e}")
...
def get_camera():
    ...
    if os.name == 'nt':  # Windows
        backends = [cv2.CAP_DSHOW, cv2.CAP_MSMF, cv2.CAP_WDS, cv2.CAP_ANY]
    ...
    print("❌ No camera found on system")
    return None
                ...
                if face_locations:
                    for face_encoding in face_encs:
                        ...
                        if True in matches:
                            ...
                            print(f"✓ Face recognized: {known_face_names[match_idx]}")
                            break
                else:
        ...
        for disk in c.Win32_LogicalDisk():
            if disk.DriveType == 2:  # Removable
                ...
                if serial:
                    ...
                    print(f"[*] Detected USB: {disk.DeviceID} (Serial: {serial})")
    ...
    try:
        ...
        try:
            for device in c.Win32_PnPEntity(ConfigManagerErrorCode=0):
                ...
                if any(x in name.lower() for x in ['phone', 'mobile', 'mtp', 'android', 'portable']):
                    devices[name] = device_id
                    print(f"[*] Detected Mobile: {name}")
        except:
            pass
        
        pythoncom.CoUninitialize()
    except Exception as e:
        print(f"⚠️  Mobile detection error: {e}")
    
    return devices

def send_security_event(action, device_name, device_id, image_path=None, face_status="Unknown"):
...
def main():
    ...
    user_id, username = get_active_user_id()
    print(f"✓ Active User: {username} (ID: {user_id})\n")
        ...
        try:
            ...
            for drive_letter, serial in new_drives.items():
                if drive_letter not in current_drives:
                    print(f"\n🔴 NEW USB DETECTED: {drive_letter} (ID: {serial})")
                    image_path, face_status = capture_instant_photo()
                    send_security_event("USB Inserted", f"USB Drive ({drive_letter})", serial, image_path, face_status)
            
            # Check for removed USB drives
            for drive_letter, serial in current_drives.items():
                if drive_letter not in new_drives:
                    print(f"\n⚪ USB REMOVED: {drive_letter}")
                    send_security_event("USB Removed", f"USB Drive ({drive_letter})", serial)
            ...
            for phone_name, device_id in new_mobiles.items():
                if phone_name not in current_mobiles:
                    ...
                    image_path, face_status = capture_instant_photo()
                    send_security_event("Mobile Connected", f"Mobile Device ({phone_name})", device_id, image_path, face_status)
            
            # Check for removed mobile devices
            for phone_name, device_id in current_mobiles.items():
                if phone_name not in new_mobiles:
                    print(f"\n📱 MOBILE REMOVED: {phone_name}")
                    send_security_event("Mobile Removed", f"Mobile Device ({phone_name})", device_id)
